    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGet()
        {
            try
            {
                ...
                var Result = Client.GetAsync("api/Branches/GetAll").Result;
                if (Result.IsSuccessStatusCode)
                {
                    var Model = Result.Content.ReadFromJsonAsync<List<BranchDto>>();
        ...
        public IActionResult OnPostDelete(string Id)
        {
            try
            {
                ...
                var token = User.FindFirst(_SettingWeb.TokenName).Value;
                Client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(_SettingWeb.TokenType, token);
                var Result = Client.DeleteAsync("api/Branches/Delete?Id=" + Id + "").Result;
                if (Result.IsSuccessStatusCode)
                {
